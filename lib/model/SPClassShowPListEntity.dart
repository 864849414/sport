  ...
  Map<String,dynamic> spProMoney2Diamond;
	...
	SPClassShowPListEntity.fromJson(Map<String, dynamic> json) {
		if (json["pay_list"] != null) {
			spProPayList = new List<SPClassShowPListPayList>();(json["pay_list"] as List).forEach((v) { spProPayList.add(new SPClassShowPListPayList.fromJson(v)); });
		}

    if (json["payed_money_list"] != null) {
      spProPayedMoneyList = new List<String>();(json["payed_money_list"] as List).forEach((v) { spProPayedMoneyList.add(v.toString()); });
    }
		if (json["money_list"] != null) {
			spProMoneyList = new List<String>();(json["money_list"] as List).forEach((v) { spProMoneyList.add(v.toString()); });
		}
		if (json["first_pay_double_money"] != null) {
			spProDoubleMoneys = new List<String>();(json["first_pay_double_money"] as List).forEach((v) { spProDoubleMoneys.add(v.toString()); });
		}
